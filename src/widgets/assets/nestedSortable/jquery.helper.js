...
// Bail if Helper is already defined
if (typeof Helper != 'undefined')
{
	throw 'Helper is already defined!';
}
...
Helper = $.extend(Helper, {
	...
	{
		return (elem.nodeType == Helper.TEXT_NODE);
	},
	...
	{
		Helper.hitTest._$elem = $(elem);
		Helper.hitTest._offset = Helper.hitTest._$elem.offset();
		Helper.hitTest._x1 = Helper.hitTest._offset.left;
		Helper.hitTest._y1 = Helper.hitTest._offset.top;
		Helper.hitTest._x2 = Helper.hitTest._x1 + Helper.hitTest._$elem.outerWidth();
		Helper.hitTest._y2 = Helper.hitTest._y1 + Helper.hitTest._$elem.outerHeight();

		return (x >= Helper.hitTest._x1 && x < Helper.hitTest._x2 && y >= Helper.hitTest._y1 && y < Helper.hitTest._y2);
	},
	...
	{
		return Helper.hitTest(ev.pageX, ev.pageY, elem);
	},
	...
	{
		Helper.getBodyScrollTop._scrollTop = document.body.scrollTop;

		if (Helper.getBodyScrollTop._scrollTop < 0)
		{
			Helper.getBodyScrollTop._scrollTop = 0;
		}
		else
		{
			Helper.getBodyScrollTop._maxScrollTop = Helper.$bod.outerHeight() - Helper.$win.height();

			if (Helper.getBodyScrollTop._scrollTop > Helper.getBodyScrollTop._maxScrollTop)
			{
				Helper.getBodyScrollTop._scrollTop = Helper.getBodyScrollTop._maxScrollTop;
			}
		}

		return Helper.getBodyScrollTop._scrollTop;
	},
	...
	shake: function(elem, prop)
	{
		...
		for (var i = 0; i <= Helper.SHAKE_STEPS; i++)
		{
			(function(i)
			{
				setTimeout(function()
				{
					Helper.shake._properties = {};
					Helper.shake._properties[prop] = startingPoint + (i % 2 ? -1 : 1) * (10-i);
					$elem.velocity(Helper.shake._properties, Helper.SHAKE_STEP_DURATION);
				}, (Helper.SHAKE_STEP_DURATION * i));
			})(i);
		}
	},
	...
	getInputPostVal: function($input)
	{
		...
		//  - e.g. a multi-select
		else if (Helper.isArray(val) && $input.attr('name').substr(-2) != '[]')
		{
			if (val.length)
			{
	...
	getPostData: function(container)
	{
		var postData = {},
			arrayInputCounters = {},
			$inputs = Helper.findInputs(container);
		...
		for (var i = 0; i < $inputs.length; i++)
		{
			...
			var inputVal = Helper.getInputPostVal($input);
			if (inputVal === null)
			{
				continue;
			}
	...
	{
		var $sourceInputs = Helper.findInputs(source),
			$targetInputs = Helper.findInputs(target);
...
function resizeListener(ev)
{
	...
	{
		Helper.cancelAnimationFrame(win.__resizeRAF__);
	}

	win.__resizeRAF__ = Helper.requestAnimationFrame(function()
	{
 ...
 */
Helper.BaseDrag = Helper.Base.extend({
	...
	init: function(items, settings)
	{
		...
		this.settings = $.extend({}, Helper.BaseDrag.defaults, settings);
	...
	drag: function(didMouseMove)
	{
		if (didMouseMove)
		{
			...
			if (this.settings.axis != Helper.X_AXIS)
			{
				// Scrolling up?
				this.drag._winScrollTop = Helper.$win.scrollTop();
				this.drag._minMouseScrollY = this.drag._winScrollTop + Helper.BaseDrag.windowScrollTargetSize;
				...
				{
					// Scrolling down?
					this.drag._maxMouseScrollY = this.drag._winScrollTop + Helper.$win.height() - Helper.BaseDrag.windowScrollTargetSize;
			...
			if (!this.drag._scrollProperty && this.settings.axis != Helper.Y_AXIS)
			{
				// Scrolling left?
				this.drag._winScrollLeft = Helper.$win.scrollLeft();
				this.drag._minMouseScrollX = this.drag._winScrollLeft + Helper.BaseDrag.windowScrollTargetSize;
				...
				{
					// Scrolling right?
					this.drag._maxMouseScrollX = this.drag._winScrollLeft + Helper.$win.width() - Helper.BaseDrag.windowScrollTargetSize;
			...
			if (this.drag._scrollProperty)
			{
				...
				if (!this.scrollProperty)
				{
					...
					if (this.scrollFrame)
					{
						Helper.cancelAnimationFrame(this.scrollFrame);
						this.scrollFrame = null;
					}

					this.scrollFrame = Helper.requestAnimationFrame(this.scrollProxy);
				}
...
//throw new Exception("test");
			...
			if ($.data(item, 'drag'))
			{
				Helper.log('Element was added to more than one dragger');
	...
	{
		// Ignore right clicks
		if (ev.which != Helper.PRIMARY_CLICK)
		{
			return;
		}
		...
		this.addListener(Helper.$doc, 'mousemove', '_handleMouseMove');
		this.addListener(Helper.$doc, 'mouseup', '_handleMouseUp');
	},
	...
	_handleMouseMove: function(ev)
	{
		...
		if (this.settings.axis != Helper.Y_AXIS)
		{
		...
		if (!this.dragging)
		{
			...
			this._handleMouseMove._mouseDist = Helper.getDist(this.mousedownX, this.mousedownY, this.realMouseX, this.realMouseY);

			if (this._handleMouseMove._mouseDist >= Helper.BaseDrag.minMouseDist)
			{
				this.startDragging();
			}
		}
	...
	{	
		...
		this.removeAllListeners(Helper.$doc);
	...
	{
		this._.scrollPos = Helper.$win[this.scrollProperty]();
		Helper.$win[this.scrollProperty](this._.scrollPos + this.scrollDist);

		this['mouse'+this.scrollAxis] -= this._.scrollPos - Helper.$win[this.scrollProperty]();
 ...
 */
Helper.CheckboxSelect = Helper.Base.extend({
	...
	init: function(container)
	{
		...
		{
			Helper.log('Double-instantiating a checkbox select on an element');
 ...
 * Context Menu
 */
Helper.ContextMenu = Helper.Base.extend({
	...
	init: function(target, options, settings)
	{
		...
		{
			Helper.log('Double-instantiating a context menu on an element');
		...
		this.setSettings(settings, Helper.ContextMenu.defaults);

		Helper.ContextMenu.counter++;
	...
	showMenu: function(ev)
	{
		...
		if (ev.type == 'mousedown' && ev.which != Helper.SECONDARY_CLICK)
		{
			return;
		}
		...
		setTimeout($.proxy(function()
		{
			this.addListener(Helper.$doc, 'mousedown', 'hideMenu');
		}, this), 0);
	},
	...
	hideMenu: function()
	{
		this.removeListener(Helper.$doc, 'mousedown');
		this.$menu.hide();
		this.showing = false;
	},
 ...
 */
Helper.Drag = Helper.BaseDrag.extend({
	...
	init: function(items, settings)
	{
		...
		settings = $.extend({}, Helper.Drag.defaults, settings);
	...
	startDragging: function()
	{
		...
		this.updateHelperPosFrame = Helper.requestAnimationFrame(this.updateHelperPosProxy);
	...
	stopDragging: function()
	{
		...
		Helper.cancelAnimationFrame(this.updateHelperPosFrame);
	...
	returnHelpersToDraggees: function()
	{
		...
		for (var i = 0; i < this.helpers.length; i++)
		{
			...
			$helper.velocity({left: draggeeOffset.left, top: draggeeOffset.top}, Helper.FX_DURATION, callback);
		}
	},
	...
	_createHelper: function(i)
	{
		...
		{
			Helper.copyInputValues($draggee, $draggeeHelper);
		}
		...
		$draggeeHelper.appendTo(Helper.$bod);
 ...
 */
Helper.DragDrop = Helper.Drag.extend({
	...
	{
		settings = $.extend({}, Helper.DragDrop.defaults, settings);
		this.base(settings);
	},
	...
	onDrag: function()
	{
		if (this.$dropTargets)
		{
			...
			for (this.onDrag._i = 0; this.onDrag._i < this.$dropTargets.length; this.onDrag._i++)
			{
				...
				if (Helper.hitTest(this.mouseX, this.mouseY, this.onDrag._elem))
				{
 ...
 */
Helper.DragMove = Helper.BaseDrag.extend({
 ...
 */
Helper.DragSort = Helper.Drag.extend({
	...
	init: function(items, settings)
	{
		...
		settings = $.extend({}, Helper.DragSort.defaults, settings);
		this.base(items, settings);
	},
	...
	onDrag: function()
	{
		...
		if (this.$heightedContainer && !Helper.hitTest(this.mouseX, this.mouseY, this.$heightedContainer))
		{
			if (this.closestItem)
			{
				this.closestItem = null;
	...
	_getClosestItem: function()
	{
		...
		if (this.settings.axis != Helper.Y_AXIS) this._getClosestItem._startXDist = this._getClosestItem._lastXDist = this._getClosestItem._closestItem ? Math.abs(this._getClosestItem._midpoint.x - this.draggeeVirtualMidpointX) : null;
		if (this.settings.axis != Helper.X_AXIS) this._getClosestItem._startYDist = this._getClosestItem._lastYDist = this._getClosestItem._closestItem ? Math.abs(this._getClosestItem._midpoint.y - this.draggeeVirtualMidpointY) : null;
		...
		while (this._getClosestItem._$otherItem.length)
		{
			...
			if (this.settings.axis != Helper.Y_AXIS) this._getClosestItem._xDist = Math.abs(this._getClosestItem._midpoint.x - this.draggeeVirtualMidpointX);
			if (this.settings.axis != Helper.X_AXIS) this._getClosestItem._yDist = Math.abs(this._getClosestItem._midpoint.y - this.draggeeVirtualMidpointY);

			if (
				(this.settings.axis == Helper.Y_AXIS || (this._getClosestItem._lastXDist !== null && this._getClosestItem._xDist > this._getClosestItem._lastXDist)) &&
				(this.settings.axis == Helper.X_AXIS || (this._getClosestItem._lastYDist !== null && this._getClosestItem._yDist > this._getClosestItem._lastYDist))
			)
			{
				break;
			}

			if (this.settings.axis != Helper.Y_AXIS) this._getClosestItem._lastXDist = this._getClosestItem._xDist;
			if (this.settings.axis != Helper.X_AXIS) this._getClosestItem._lastYDist = this._getClosestItem._yDist;
		...
		if (this.settings.axis != Helper.Y_AXIS) this._getClosestItem._lastXDist = this._getClosestItem._startXDist;
		if (this.settings.axis != Helper.X_AXIS) this._getClosestItem._lastYDist = this._getClosestItem._startYDist;
 ...
 */
Helper.EscManager = Helper.Base.extend({
	...
	init: function()
	{
		...
		this.addListener(Helper.$bod, 'keyup', function(ev)
		{
...
Helper.escManager = new Helper.EscManager();


/**
 * HUD
 */
Helper.HUD = Helper.Base.extend({
	...
	init: function(trigger, bodyContents, settings) {
		...
		this.setSettings(settings, Helper.HUD.defaults);
		this.on('show', this.settings.onShow);
		this.on('hide', this.settings.onHide);

		if (typeof Helper.HUD.activeHUDs == "undefined")
		{
			Helper.HUD.activeHUDs = {};
		}
	...
	show: function(ev)
	{
		...
		{
			for (var hudID in Helper.HUD.activeHUDs)
			{
				Helper.HUD.activeHUDs[hudID].hide();
			}
		}

		// Prevent the browser from jumping
		this.$hud.css('top', Helper.$win.scrollTop());

		// Move it to the end of <body> so it gets the highest sub-z-index
		this.$shade.appendTo(Helper.$bod);
		this.$hud.appendTo(Helper.$bod);
		...
		this.showing = true;
		Helper.HUD.activeHUDs[this._namespace] = this;

		Helper.escManager.register(this, 'hide');

		this.addListener(this.$hud, 'resize', 'resetPosition');
		this.addListener(Helper.$win, 'resize', 'resetPosition');
	...
	{
		this.windowWidth = Helper.$win.width();
		this.windowHeight = Helper.$win.height();

		this.windowScrollLeft = Helper.$win.scrollLeft();
		this.windowScrollTop = Helper.$win.scrollTop();
	...
	determineBestPosition: function()
	{
		...
		this.position = Helper.HUD.positions[positionIndex];
		...
		this.tipClass = this.settings.tipClass+'-'+Helper.HUD.tipClasses[positionIndex];
	...
	hide: function()
	{
		...
		delete Helper.HUD.activeHUDs[this._namespace];

		Helper.escManager.unregister(this);
 ...
 * Light Switch
 */
Helper.LightSwitch = Helper.Base.extend({
	...
	init: function(outerContainer, settings)
	{
		...
		{
			Helper.log('Double-instantiating a switch on an element');
			this.$outerContainer.data('lightswitch').destroy();
		}

		this.$outerContainer.data('lightswitch', this);

		this.setSettings(settings, Helper.LightSwitch.defaults);
		...
		this.dragger = new Helper.BaseDrag(this.$outerContainer, {
			axis:                 Helper.X_AXIS,
	...
	turnOn: function()
	{
		this.$innerContainer.velocity('stop').velocity({marginLeft: 0}, Helper.FX_DURATION);
		...
		this.$toggleTarget.velocity('stop').velocity({height: height}, Helper.FX_DURATION, $.proxy(function() {
			this.$toggleTarget.height('auto');
		}, this));
	},
	...
	turnOff: function()
	{
		this.$innerContainer.velocity('stop').velocity({marginLeft: Helper.LightSwitch.offMargin}, Helper.FX_DURATION);
		this.$input.val('');
		this.on = false;
		this.onChange();

		this.$toggleTarget.velocity('stop').velocity({height: 0}, Helper.FX_DURATION);
	},
	...
	{
		this.removeListener(Helper.$doc, 'mouseup');
	...
	_handleDrag: function()
	{
		...
		if (margin < Helper.LightSwitch.offMargin)
		{
			margin = Helper.LightSwitch.offMargin;
		}
		else if (margin > 0)
		{
			margin = 0;
 ...
 * Menu
 */
Helper.Menu = Helper.Base.extend({
	...
	{
		this.setSettings(settings, Helper.Menu.defaults);
	...
	{
		this._windowWidth = Helper.$win.width();
		this._windowHeight = Helper.$win.height();
		this._windowScrollLeft = Helper.$win.scrollLeft();
		this._windowScrollTop = Helper.$win.scrollTop();
	...
	show: function()
	{
		...
		this.$container.appendTo(Helper.$bod)
	...
	hide: function()
	{
		this.$container.velocity('fadeOut', { duration: Helper.FX_DURATION }, $.proxy(function()
		{
 ...
 * Menu Button
 */
Helper.MenuBtn = Helper.Base.extend({
	...
	init: function(btn, settings)
	{
		...
		if (this.$btn.data('menubtn'))
		{
			...
			Helper.log('Double-instantiating a menu button on an element');
		...
		this.setSettings(settings, Helper.MenuBtn.defaults);

		this.menu = new Helper.Menu($menu, {
	...
	{
		if (ev.which != Helper.PRIMARY_CLICK || ev.metaKey)
		{
			return;
		}
	...
	showMenu: function()
	{
		...
		setTimeout($.proxy(function() {
			this.addListener(Helper.$doc, 'mousedown', 'onMouseDown');
		}, this), 1);

		if (!Helper.isMobileBrowser())
		{
			this.addListener(Helper.$win, 'resize', 'hideMenu');
		}
	},
	...
	hideMenu: function()
	{
		if (!Helper.isMobileBrowser())
		{
			this.removeListener(Helper.$win, 'resize');
		}
	...
	onMenuHide: function()
	{
		...
		if (!Helper.isMobileBrowser())
		{
			this.removeListener(Helper.$doc, 'resize');
		}
	},
 ...
 */
Helper.MixedInput = Helper.Base.extend({
	...
	{
		this.$container = $(container);
		this.setSettings(settings, Helper.MixedInput.defaults);
...
var TextElement = Helper.Base.extend({
	...
	buildStage: function()
	{
		this.$stage = $('<stage/>').appendTo(Helper.$bod);
		...
		Helper.copyTextStyles(this.$input, this.$stage);
	},
	...
	{
		this.focussed = true;
		this.interval = setInterval($.proxy(this, 'checkInput'), Helper.NiceText.interval);
		this.checkInput();
	},
 ...
 * Modal
 */
Helper.Modal = Helper.Base.extend({
	...
	init: function(container, settings)
	{
		...
		this.setSettings(settings, Helper.Modal.defaults);
		...
		Helper.Modal.instances.push(this);
	},
	...
	setContainer: function(container)
	{
		...
		{
			Helper.log('Double-instantiating a modal on an element');
		...
		{
			this.dragger = new Helper.DragMove(this.$container, {
		...
		if (this.settings.resizable)
		{
			...
			this.resizeDragger = new Helper.BaseDrag($resizeDragHandle, {
	...
	show: function()
	{
		...
		if (this.settings.closeOtherModals && Helper.Modal.visibleModal && Helper.Modal.visibleModal != this)
		{
			Helper.Modal.visibleModal.hide();
		}
		...
		if (this.$container)
		{
			...
			this.addListener(Helper.$win, 'resize', 'updateSizeAndPosition');
		}
		...
		{
			this.visible = true;
			Helper.Modal.visibleModal = this;
	...
	hide: function(ev)
	{
		...
		if (this.$container)
		{
			this.$container.velocity('fadeOut', { duration: Helper.FX_DURATION });
		...
		this.visible = false;
		Helper.Modal.visibleModal = null;
	...
	updateSizeAndPosition: function()
	{
		...
		this.updateSizeAndPosition._windowWidth = Helper.$win.width();
		...
		// Now set the height
		this.updateSizeAndPosition._windowHeight = Helper.$win.height();
 ...
 * Nice Text
 */
Helper.NiceText = Helper.Base.extend({
	...
	{
		this.$input = $(input);
		this.settings = $.extend({}, Helper.NiceText.defaults, settings);
		...
		if (this.maxLength && (this.settings.showCharsLeft || Helper.hasAttr(this.$input, 'data-show-chars-left')))
		{
		...
		{
			Helper.log('Double-instantiating a transparent text input on an element');
		...
		if (this.autoHeight)
		{
			...
			this.addListener(Helper.$win, 'resize', 'updateHeight');
		}
		...
		if (this.settings.hint)
		{
			...
			Helper.copyTextStyles(this.$input, this.$hint);
	...
	showHint: function()
	{
		this.$hint.velocity('fadeIn', {
			complete: Helper.NiceText.hintFadeDuration
		});
 ...
 * Pill
 */
Helper.Pill = Helper.Base.extend({
	...
	init: function(outerContainer)
	{
		...
		{
			Helper.log('Double-instantiating a pill on an element');
		...
		Helper.preventOutlineOnMouseFocus(this.$innerContainer);
