...
EOT;
    ...
    public function selectOptionForSelect2($selector, $option, $timeout = 5)
    {
        ...
        if (is_string($option) || (is_array($option) && array_values($option) === $option)) {
            $I->executeJS('jQuery("'.$selector.'").select2("val", '.json_encode($option).');');
            $I->executeJS('jQuery("'.$selector.'").trigger("select2:select").trigger("change");');
...
EOT;
            ...
            $I->executeJS('jQuery("'.$selector.'").select2("val", "");');
            $I->executeJS('jQuery("'.$selector.'").trigger("select2:select").trigger("change");');
        }
    }
